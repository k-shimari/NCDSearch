...
public class TokenLevenshteinDistance implements ICodeDistanceStrategy {
	...
	protected int[][] computeDistanceTable(TokenSequence code ,TokenSequence code2) {
		int[][] distance = new int[code.size() + 1][code2.size() + 1];
		for (int i = 0; i < code.size() + 1; i++) {
			distance[i][0] = i;
		}
		for (int j = 0; j < code2.size() + 1; j++) {
			distance[0][j] = j;
		}

		for (int i = 1; i < code.size() + 1; i++) {
			String t1 = code.getToken(i - 1);
			for (int j = 1; j < code2.size() + 1; j++) {
				String t2 = code2.getToken(j - 1);
	...
	public double computeDistance(TokenSequence code, TokenSequence code2) {
		int[][] distance = computeDistanceTable(code2);
		return distance[code.size()][code2.size()];
	}
